...
bool CSocketManager::Start(WORD port) // OK
{
	...
	gUtil.LogAdd(LOG_BLACK, "[SocketManager] Server started at port [%d]", this->m_port);
	return 1;
}
...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] WSASocket() failed with error: %d", WSAGetLastError());
		return 0;
	}
	...
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] bind() failed with error: %d", WSAGetLastError());
		return 0;
	}

	if (listen(this->m_listen, 5) == SOCKET_ERROR)
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] listen() failed with error: %d", WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::CreateCompletionPort() // OK
{
	...
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] socket() failed with error: %d", WSAGetLastError());
		return 0;
	}

	if ((this->m_CompletionPort = CreateIoCompletionPort((HANDLE)socket, 0, 0, 0)) == 0)
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] CreateIoCompletionPort() failed with error: %d", GetLastError());
		closesocket(socket);
		return 0;
	}
...
bool CSocketManager::CreateAcceptThread() // OK
{
	...
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] CreateThread() failed with error: %d", GetLastError());
		return 0;
	}

	if (SetThreadPriority(this->m_ServerAcceptThread, THREAD_PRIORITY_HIGHEST) == 0)
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] SetThreadPriority() failed with error: %d", GetLastError());
		return 0;
	}
...
bool CSocketManager::CreateServerQueue() // OK
{
	...
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] CreateSemaphore() failed with error: %d", GetLastError());
		return 0;
	}
...
bool CSocketManager::DataRecv(int index, IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while (true)
	{
		...
		{
			gUtil.LogAdd(LOG_RED, "[SocketManager] Protocol header error (Index: %d, Header: %x)", index, lpMsg[count]);
			return 0;
		}

		if (size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			gUtil.LogAdd(LOG_RED, "[SocketManager] Protocol size error (Index: %d, Header: %x, Size: %d, Head: %x)", index, header, size, head);
			return 0;
		}
...
bool CSocketManager::DataSend(int index, BYTE* lpMsg, int size) // OK
{
	...
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] Max msg size (Type: 1, Index: %d, Size: %d)", index, size);
		this->m_critical.unlock();
		return 0;
	}
	...
	if (lpIoContext->IoSize > 0)
	{
		...
		{
			gUtil.LogAdd(LOG_RED, "[SocketManager] Max msg size (Type: 2, Index: %d, Size: %d)", index, (lpIoContext->IoSideBuffer.size + size));
	...
	if (WSASend(lpServerManager->m_socket, &lpIoContext->wsabuf, 1, &SendSize, Flags, &lpIoContext->overlapped, 0) == SOCKET_ERROR)
	{
		...
		{
			gUtil.LogAdd(LOG_RED, "[SocketManager] WSASend() failed with error: %d", WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}
...
void CSocketManager::Disconnect(int index) // OK
{
	...
	{
		gUtil.LogAdd(LOG_RED, "[SocketManager] closesocket() failed with error: %d", WSAGetLastError());
		this->m_critical.unlock();
		return;
	}
...
void CSocketManager::OnRecv(int index, DWORD IoSize, IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if (WSARecv(lpServerManager->m_socket, &lpIoContext->wsabuf, 1, &RecvSize, &Flags, &lpIoContext->overlapped, 0) == SOCKET_ERROR)
	{
		if (WSAGetLastError() != WSA_IO_PENDING)
		{
			gUtil.LogAdd(LOG_RED, "[SocketManager] WSARecv() failed with error: %d", WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
		}
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while (true)
	{
		...
		if (socket == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK)
		{
			...
			gUtil.LogAdd(LOG_RED, "[SocketManager] WSAAccept() failed with error: %d", WSAGetLastError());
		...
		if ((index = gUtil.GetFreeServerIndex()) == -1)
		{
			closesocket(socket);
			lpSocketManager->m_critical.unlock();
			continue;
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager) // OK
{
	...
	while (true)
	{
		if (GetQueuedCompletionStatus(lpSocketManager->m_CompletionPort, &IoSize, &index, &lpOverlapped, INFINITE) == 0)
		{
			...
			{
				lpSocketManager->m_critical.lock();
				gUtil.LogAdd(LOG_RED, "[SocketManager] GetQueuedCompletionStatus() failed with error: %d", GetLastError());
				lpSocketManager->m_critical.unlock();
				return 0;
			}
		}
...
DWORD WINAPI CSocketManager::ServerQueueThread(CSocketManager* lpSocketManager) // OK
{
	while (true)
	{
		...
		{
			gUtil.LogAdd(LOG_RED, "[SocketManager] WaitForSingleObject() failed with error: %d", GetLastError());
			break;
		}
		...
		if (lpSocketManager->m_ServerQueue.GetFromQueue(&QueueInfo) != 0)
		{
			...
			{
				ProtocolCore(QueueInfo.index, QueueInfo.head, QueueInfo.buff, QueueInfo.size);
			}
		}
	}
