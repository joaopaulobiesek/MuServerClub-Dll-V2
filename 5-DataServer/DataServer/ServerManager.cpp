...
void CServerManager::AddServer(int index, char* ip, SOCKET socket) // OK
{
	...
	gUtil.gServerCount = ((this->CheckAlloc() == 0) ? (((++gUtil.gServerCount) >= MAX_SERVER) ? 0 : gUtil.gServerCount) : gUtil.gServerCount);
	...
	gUtil.LogAdd(LOG_BLACK, "[ServerManager][%d] AddServer (%s)", this->m_index, this->m_IpAddr);
}

void CServerManager::DelServer() // OK
{
	gUtil.LogAdd(LOG_BLACK, "[ServerManager][%d] DelServer (%s)", this->m_index, this->m_IpAddr);
