...
void CServerDisplayer::SetWindowName() // OK
{
	...
	wsprintf(buff,"[%s] AuthServerDll (QueueSize : %d)",AUTHSERVER_VERSION,gSocketManager.GetQueueSize());
	...
	int iStatusWidths[] = { 125, 600 };

	char text[256];

	SendMessage(hWndStatusBar, SB_SETPARTS, 2, (LPARAM)iStatusWidths);

	wsprintf(text, "UserCount : %d/5000", GetUserCount());

	SendMessage(hWndStatusBar, SB_SETTEXT, 0, (LPARAM)text);

	wsprintf(text, "IP: %s - Port: %d", m_IpAddress, gSocketManager.m_port);
