...
void CLog::AddLog(BOOL active, const char* directory) // OK
{
...
void CLog::AddLogHack(BOOL active, const char* directory) // OK
{
	if (this->m_count2 < 0 || this->m_count2 >= MAX_LOG2)
	{
		return;
	}

	LOG_INFO* lpInfo = &this->m_LogInfo2[this->m_count2++];
...
void CLog::AddLogHackSpeed(BOOL active, const char* directory) // OK
{
	if (this->m_count2 < 0 || this->m_count2 >= MAX_LOG2)
	{
		return;
...
void CLog::OutputHack(eLogType type, const char* text, ...) // OK
{
	if (type < 0 || type >= this->m_count2)
	{
		return;
	}

	LOG_INFO* lpInfo = &this->m_LogInfo2[type];
...
void CLog::OutputHackSpeed(eLogType type, const char* text, ...) // OK
{
	if (type < 0 || type >= this->m_count2)
	{
		return;
