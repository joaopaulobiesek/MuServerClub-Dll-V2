...
void CConnection::CHClientInfoSend() // OK
{
	VM_TIGER_BLACK_START
		...
		SERVERDLL_CLIENT_LICENSED_SEND pMsg;

	pMsg.header.set(0x00, sizeof(pMsg));

	pMsg.UserId = gServerInfo.LicenseId;

	memcpy(pMsg.IpAddress, gServerInfo.IpAddress, sizeof(gServerInfo.IpAddress));

	memcpy(pMsg.CustomerHardwareId, gServerInfo.HardwareId, sizeof(gServerInfo.HardwareId));
...
void CConnection::PacketDecryptDataAuth(BYTE* lpMsg, int size, BYTE key) // OK
{
	VM_TIGER_BLACK_START
		...
		{
			lpMsg[n] = (lpMsg[n] ^ (key + 0x483)) - 0xF95;
		}
	VM_TIGER_BLACK_END
}
...
void CConnection::PacketEncryptDataAuth(BYTE* lpMsg, int size, BYTE key) // OK
{
	VM_TIGER_BLACK_START
		for (int n = 0; n < size; n++)
		{
			lpMsg[n] = (lpMsg[n] + 0xF95) ^ (key + 0x483);
		}
	VM_TIGER_BLACK_END
}
...
bool CConnection::DataRecv() // OK
{
	...
	while (true)
	{
		...
		if (size <= this->m_RecvSize)
		{
			if (header == 0xC1)
			{
				PacketDecryptDataAuth(&this->m_RecvBuff[count + 4], (size - 4), this->m_RecvBuff[count + 3]);
				head = this->m_RecvBuff[count + 3];
			}
			else
			{
				PacketDecryptDataAuth(&this->m_RecvBuff[count + 3], (size - 3), this->m_RecvBuff[count + 2]);
				head = this->m_RecvBuff[count + 3];
			}
...
bool CConnection::DataSend(BYTE* lpMsg, int size) // OK
{
	...
	if (buff[0] == 0xC1)
	{
		PacketEncryptDataAuth(&buff[4], (size - 4), buff[3]);
	}
	else
	{
		PacketEncryptDataAuth(&buff[3], (size - 3), buff[2]);
	}
