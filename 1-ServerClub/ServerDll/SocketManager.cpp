...
void CSocketManager::ReturnIp() // OK
{
	...
	if (strcmp(gServerInfo.IpAddressExt, "0") == 0)
	{
		wsprintf(gServerInfo.IpAddress, "%s", ip);
	}
	else
	{
		wsprintf(gServerInfo.IpAddress, "%s", gServerInfo.IpAddressExt);
	}
...
bool CSocketManager::Start(WORD port) // OK
{
	...
	GetHardwareId(HWID);
	this->ReturnIp();
	LogAdd(LOG_BLACK, "HWID Server: %s", HWID);
	return 1;
}
...
int CALLBACK CSocketManager::ServerAcceptCondition(IN LPWSABUF lpCallerId, IN LPWSABUF lpCallerData, IN OUT LPQOS lpSQOS, IN OUT LPQOS lpGQOS, IN LPWSABUF lpCalleeId, OUT LPWSABUF lpCalleeData, OUT GROUP FAR* g, CSocketManager* lpSocketManager) // OK
{
	...
	//Aqui ficava a verificação de quantos IP tinha conectado agora é feito por script.
	/*if (gIpManager.CheckIpAddress(inet_ntoa(SocketAddr->sin_addr), 1) == 0)
	{
		return CF_REJECT;
	}
	else
...
DWORD WINAPI CSocketManager::ServerQueueThread(CSocketManager* lpSocketManager) // OK
{
	while (true)
	{
		...
		if (lpSocketManager->m_ServerQueue.GetFromQueue(&QueueInfo) != 0)
		{
			...
			{
				ProtocolCoreMain(QueueInfo.index, QueueInfo.head, QueueInfo.buff, QueueInfo.size);
			}
		}
	}
