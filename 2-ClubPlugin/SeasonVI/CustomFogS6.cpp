...
CUSTOM_FOG* cFog::GetFogByMap(int MapNumber)
{
	for (int n = 0; n < MAX_FOG; n++)
	{
		...
		if (lpInfo->MapNumber == MapNumber && lpInfo->Enable == 1)
		{
			return lpInfo;
		}
	}
...
void cFog::glMainEnable(GLenum cap)
{
	...
	if (gFog.Anisotropy != 0)
	{
		glGetIntegerv(0x84FF, &gFog.MaxAnisotropy);
		glTexParameteri(GL_TEXTURE_2D, 0x84FE, gFog.MaxAnisotropy - 0.1);
	}
	if (gFog.Linear != 0)
	{
	...
	if (*(DWORD*)(MAIN_SCREEN_STATE_S6) == GameProcess)
	{
...
{
	if (gFog.EnableFog && gFog.Fog && *(DWORD*)(MAIN_SCREEN_STATE_S6) == GameProcess_2)
	{
		CUSTOM_FOG* lpFog = gFog.GetFogByMap(pMapNumber);
...
void WINAPI glEnable_hook(GLenum mode)
{
	...
	if (gFog.Fog == 1 && *(DWORD*)(MAIN_SCREEN_STATE_S6) == GameProcess_2)
	{
		CUSTOM_FOG* lpFog = gFog.GetFogByMap(pMapNumber);
...
void cFog::Scan()
{
	...
	for (std::vector<CUSTOM_FOG>::iterator it = gListManager.gCustomFogListInfo.begin(); it != gListManager.gCustomFogListInfo.end(); it++)
	{
		info.Index = it->Index;

		info.MapNumber = it->MapNumber;

		info.Enable = it->Enable;
